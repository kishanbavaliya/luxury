...
trait FetchDriversFromFirebaseHelpers
{
     ...
     */
    //
    protected function fetchDriversFromFirebase($request_detail, $only_luxury_limoexpress = 0)
    {
        ...
        if($only_luxury_limoexpress == 1){
            $Owner = Owner::where("id", luxury_limoexpress_owner_id())->first();
            $notifable_owner = $Owner->user;

            $title = trans('push_notifications.new_request_title',[],$notifable_owner->lang);
            $body = trans('push_notifications.new_request_body',[],$notifable_owner->lang);
            $push_data = ['title' => $title,'message' => $body,'push_type'=>'meta-request'];
            try {
                dispatch(new SendPushNotification($notifable_owner,$title,$body,$push_data));
            } catch (\Exception $e) {
                
            }
        } else {
            if($only_luxury_limoexpress == 2){
                $Owners = Owner::where("id", "!=", luxury_limoexpress_owner_id())->get();
            } else {
                $NotIcludeOwners = NotIcludeOwner::where("request_id", $request_detail->id)->pluck("user_id")->toArray();
                if(!empty($NotIcludeOwners)){
                    $Owners = Owner::whereNotIn("user_id", $NotIcludeOwners)->get();
                } else {
                    $Owners = Owner::get();
                }
            }
        ...
        // $notifable_owner = User::where("id", 22)->first();
        ...
        // dispatch(new SendPushNotification($notifable_owner,$title,$body,$push_data));
        // \Log::info('Notfication send user id 22');
        ...
        $current_date = Carbon::now();
         ...
         if (!empty($firebase_drivers)) {
            ...
            $removable_driver_ids=[];
                ...
                {
               $has_enabled_my_route_drivers=Driver::where('id',$key)->where("available",1)->where('approve', 1)->where('enable_my_route_booking',1)->first();
                }else{
                $has_enabled_my_route_drivers=Driver::where('id',$key)->where('available', 1)->where('approve', 1)->where(function($query)use($request_detail){
            ...
            // if(count($nearest_driver_ids)==0){
                ...
                if($only_luxury_limoexpress == 1){
                    ...
                    if($only_luxury_limoexpress == 2){
                        ...
                        // $nearest_drivers = Driver::where('available', 1)->where('approve', 1)->where("owner_id", "!=", luxury_limoexpress_owner_id())->get();
                        
                        $nearest_drivers = Driver::where('available', '1')
                        ->where('approve', '1')
                        ...
                        $nearest_drivers = Driver::where('available', 1)->where('approve', 1)->limit(10)->get();
                    }
                }
                ...
                foreach ($nearest_drivers as $driver_key => $driver) {
                    ...
                    \Log::info('user_detail user id '. $user_detail->id);
