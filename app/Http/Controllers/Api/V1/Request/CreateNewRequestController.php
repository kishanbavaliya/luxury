...
class CreateNewRequestController extends BaseController
{
    ...
    *
    */
    public function updateRequest(ValidatorRequest $request){

        $Request = Request::where("id", $request->request_id)->first();
        $driver = Driver::find($Request->driver_id);
        if(!empty($driver)){

            $driver = Driver::find($Request->driver_id);
            $original_pickup_time = $Request->trip_start_time;
            $Request->accepted_at = NULL;
            $Request->driver_id = NULL;
            $Request->old_driver_id = $driver->id;
            $Request->is_driver_started = 0;
            $Request->trip_start_time = $request->trip_start_time;
            $Request->save();
            ...
            $details = [
                ...
                "original_pickup_time" => $original_pickup_time != "" ? date("d-m-Y H:i:s", strtotime($original_pickup_time)) : "",
                "updated_pickup_time" => $Request->trip_start_time != "" ? date("d-m-Y H:i:s", strtotime($Request->trip_start_time)) : "",
                "pickup_location" => $Request->pick_address,
                "destination" => $Request->drop_address,
                "vehicle_class" => $Request->vehicle_type_name,
                "booking_number" => $Request->request_number,
            ];
            \Mail::to($driver->email)->send(new \App\Mail\UpdateRequestMailForDriver($details));
            if(!empty($driver->owner)){
                $details["partner_name"] = $driver->owner->owner_name;
                \Mail::to($driver->owner->email)->send(new \App\Mail\UpdateRequestMailForDriver($details));
            }
            ...
            \Mail::to($driver->email)->send(new \App\Mail\UpdateRequestMailForCustomer($details));
    
            $notifable_driver = $driver->user;
            $title = trans('push_notifications.update_request_title',[],$notifable_driver->lang);
            $body = trans('push_notifications.update_request_body',[],$notifable_driver->lang);
...
Log::info("create-request");
        ...
        */
        ...
        $request_params = [
            ...
            // 'seats_accessories_amount'=>$request->seats_accessories_amount,
        ...
        $nearest_drivers =  $this->fetchDriversFromFirebase($request_detail, 1);
...
// dd($request_detail);
    ...
    public function driver_no_show(ValidatorRequest $request){
